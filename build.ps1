...
if ($IsWindows) {
    Copy-Item "$cppRuntimePath/*.dll" $outputPath -Recurse -ErrorAction SilentlyContinue
    Copy-Item "$cppRuntimePath/*.lib" $outputPath -Recurse -ErrorAction SilentlyContinue
} elseif ($IsLinux) {
    Copy-Item "$cppRuntimePath/*.so" $outputPath -Recurse -ErrorAction SilentlyContinue
    Copy-Item "$cppRuntimePath/*.a" $outputPath -Recurse -ErrorAction SilentlyContinue
} elseif ($IsMacOS) {
    Copy-Item "$cppRuntimePath/*.dylib" $outputPath -Recurse -ErrorAction SilentlyContinue
    Copy-Item "$cppRuntimePath/*.a" $outputPath -Recurse -ErrorAction SilentlyContinue
}
